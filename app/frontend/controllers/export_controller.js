...
module.exports.saveWordDocument = async function(title, verses, bibleBooks=undefined, notes={}) {
  ...
  const footers = await addBibleTranslationInfo(docx);
...
async function addBibleTranslationInfo(docx) {
  const bibleTranslationId = app_controller.tab_controller.getTab().getBibleTranslationId();
  ...
  const text = [
    `${i18n.t("general.scripture-quote-from")} `,
    new docx.TextRun({text: swordModule.description, bold: true}),
    swordModule.distributionLicense ? ` (${swordModule.distributionLicense})` : '',
  ];
  ...
  return {
    default: new docx.Footer({
      children: [ 
        ...
        copyright ? new docx.Paragraph(copyright) : undefined
      ]
    })
  };
}